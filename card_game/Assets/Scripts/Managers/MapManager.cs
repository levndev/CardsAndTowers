...
public class MapManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            var child = Walls.transform.GetChild(i);
            if (child.GetComponent<SizeData>() == null)
                continue;
        ...
        for (var i = 0; i < Entities.transform.childCount; i++)
        {
            var child = Entities.transform.GetChild(i);
            if (child.GetComponent<SizeData>() == null)
                continue;
            var size = child.GetComponent<SizeData>().Size;
            var mapPosition = WorldToMap(child.transform.position - new Vector3(size.x / 2.0f - 0.5f, size.y / 2.0f - 0.5f, 0));
    ...
    public GameObject AddObject(GameObject obj, Vector3 position, bool regeneratePaths = true)
    {
        ...
        obj.transform.parent = Entities.transform;
        for (var x = mapPosition.x; x < mapPosition.x + size.x; x++)
        {
            for (var y = mapPosition.y; y < mapPosition.y + size.y; y++)
            {
    ...
    private List<(Vector2Int, int)> GetNeighbours(Vector2Int position)
    {
        ...
        for (var i = 0; i < neighbours.Length; i++)
        {
            ...
            if (levelMap.InBounds(neighbour))
            {
                ...
                if (obj != null && obj.tag == "Indestructible")
                {
                    continue;
                }
